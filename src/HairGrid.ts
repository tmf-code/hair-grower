...
const jitter = (jitterRange: number) =>
  new Vector().set(
    randRange(-jitterRange, jitterRange),
    randRange(-jitterRange, jitterRange)
  );
...
export class HairGrid {
  ...
  constructor(screenWidth: number, screenHeight: number) {
    ...
    const cellWidth = 0.010 * maxDimension;
    const cellHeight = 0.010 * maxDimension;
    ...
    const positions = grid(
      ...
      cellWidth,
      cellHeight
    ).map(position => Vector.add(position, jitter(positionJitterRange)));
