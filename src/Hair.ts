...
export class Hair {
  ...
  shape(
    ...
    const rootLeft = rootPosition
      .copy()
      .sub(rootOffset)
      .array()
      .slice(0, 2);
    const rootRight = rootPosition
      .copy()
      .add(rootOffset)
      .array()
      .slice(0, 2);
    ...
    const tipLeft = tipPosition
      .copy()
      .sub(tipOffset)
      .array()
      .slice(0, 2);

    const tipRight = tipPosition
      .copy()
      .add(tipOffset)
      .array()
      .slice(0, 2);
  ...
  collideWith(
    ...
    const rotateHair = hair.rotate(
      fractionAngle *
        fractiondistanceToMouse *
        Math.max(Mouse.VelocityVector().mag(), 0) *
        10.0
    );
